...
#[put("/v1/client-price-levels/{client_id}")]
pub async fn api_v1_update_client_price_level(
    ...
    match assign_price_level_to_client(repo.get_ref(), &user, client_id, payload.into_inner()) {
        ...
        Err(err) => {
            log::error!("Failed to assign price level to client {client_id}: {err}");
            HttpResponse::InternalServerError().finish()
        }
    }
}
